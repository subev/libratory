...
// The total is read separately from the breakdown because the two now live apart: the number labels
// a menu item, the breakdown is the modal that item opens.
export function useDiskUsageTotal(bookId: string): string | null {
  const { data } = trpc.books.diskUsage.useQuery({ bookId }, { staleTime: 30_000, refetchOnWindowFocus: false });
  return data ? formatBytes(data.total) : null;
}

export function DiskUsageModal({ bookId, onClose }: { bookId: string; onClose: () => void }) {
  ...
  return (
    <Modal size="sm" onClose={onClose} backdropTestId="disk-usage-modal">
      <ModalHeader title="Disk usage" onClose={onClose} />
