...
- **Persistence**: none — the transcript is ephemeral `useChat` state (the localStorage mirror was reverted; ephemeral state + explicit save is the pattern). Durable output = "Save as note" → `notes` row with `bookId NULL`, listed in the page's "Saved answers" section (`notes.listLibrary`).
