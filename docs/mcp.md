...
wait_for_book { id, until: "output" }
              → { satisfied: true, book: { outputPath: ".../Dune_….m4b", chapters: { total: 48, withAudio: 48, … } } }
```
...
| `upload_book` | Create a book from absolute PDF paths on the machine running Libratory (copied in). Options: `title`, `voice`, `speed`, `language`, `profile`, `folder`, `fullExtract` (default true), `skipSynthesis`, `llmChapterDetection` + `chapterModel`, `ocrEngine` (`tesseract`, `surya`, or `llm` for a cloud vision model) + `ocrModel`. Refuses full extraction while the models are not installed. |
...
| `set_book_settings` | Title, `folder` (moves the book; `null` for the top level), voice, speed, language, author, OCR engine and its vision model (`ocrModel`), AI chapter detection after upload. |
