...
The port is published on **127.0.0.1 deliberately**: there is no login, so anyone who can reach it can read and delete everything. To serve your LAN, override the mapping in a `docker-compose.override.yml` — and know who is on that network — or front it with a reverse proxy or Tailscale.
