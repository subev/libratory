...
**Propose (LLM) button** (structure modal) uses a different path: `workers/propose.ts` → `lib/toc-detect.ts` calls the selected model (`books.chapterModel`, per source file): the TOC read covers the first/last 15 pages of the PDF page range (`pdftotext -layout` merged with marker blocks, so OCR books and blockless TOC pages both work) and returns entries with indentation levels; a tier call picks the chapter-level entries with cleaned titles (OCR artifacts fixed); placement is deterministic where a heading matches on the expected page and a resolve call handles the garbled rest. The parsed TOC is stored as `chapterProposal.toc` (only for files whose selection was kept) and shown in the modal banner. Proposal titles flow through apply: `applyChapterBoundaries` accepts optional per-boundary `title` overrides passed to `sliceChaptersAtIndices`. No `max_tokens` on these calls — DeepSeek Flash spends budget on reasoning first and a cap can produce an empty response; reasoning is off, which took the same TOC from 284s to 10s, timeout 600s.
