...
Every upload extracts raw text with `pdftotext` in seconds (stored per file in `book_files.raw_text`); the slow Marker extraction is **opt-in** and started from the book page (`books.extractChapters`, or the Extract modal that the upload dialog lands on with `?extract=1`); the upload dialog itself asks only whether several files are one book or many. Raw-only files carry `book_files.status = "raw"` and are skipped by the extract worker until flipped to `pending`. Whole-book Ask AI (`books.aiPromptRaw`) and the upload-time AI prompt run against the concatenated raw text; every AI answer is auto-saved to the `notes` table.
...
packages/web/src/
  ...
  pages/
    ...
    Chat.tsx            Library chat shell: top bar, history sidebar/drawer, one ConversationPane per conversation (/chat, /chat/:conversationId)
...
**Frontend** (`pages/Chat.tsx`, `components/chat/`): a viewport shell — pinned top bar, history sidebar (`ChatSidebar.tsx`; a drawer under `md`), and `ConversationPane.tsx` with its own pinned sources bar, scroller and composer. Routes `/chat` (new; `?bookId=`/`?folderId=` preselect the source, and a book also starts the history filter) and `/chat/:conversationId`. The pane is keyed by conversation, except that a chat born in it keeps its key when the first question gives it an id and a URL mid-stream — remounting there would drop the answer. `useChat` + `DefaultChatTransport` with `prepareSendMessagesRequest` sending only the new question; the conversation id and model ride in each request's body. `chats.get` is read once per opening (`gcTime: 0`) because the pane seeds `useChat` from it and then owns the transcript. History filtering, grouping and the scope summaries are pure in `lib/chat-history.ts`; sidebar filters are form fields in the sidebar and a chat's sources are chips in its own header, so the two never read as the same control. `SourcePicker.tsx` shows a capped list with a type-to-narrow field (title or author) — libraries run to hundreds of books. **New chat pressed inside a conversation starts on that conversation's sources** (`draftFromScope`, minus anything since removed), with the picker open and the carried books ticked first: re-picking books for every chat was a chore, while widening is one click ("clear", or Whole library). Tool calls rendered as collapsed "Searched: …" lines, `[c_N]` markers rewritten to `[n]` numbering matching the source list (`SourceList.tsx`: grouped under the book's title, each row the chapter and page plus how the passage starts — a dozen citations of one chapter were a dozen identical truncated pills). Row targets: a narrated chapter → the reader at the cue, `/books/:id/read?chapter=<index>&t=<ms>`, with the PDF kept beside it as a second button; otherwise raw or chapter with a resolvable file+page → `PdfPreviewModal` (`/pdf/:fileId#page=N`), else `/books/:bookId`; translation/variant → `/books/:bookId?variant=<key>`. `?t=` is where an unplayed chapter starts in `Reader.tsx` — the follow-scroll lands on whatever cue holds that moment.
