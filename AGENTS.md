...
0d. **ocrTextLayer** (`workers/ocr-text-layer.ts`, extraction pool) — Per file with no text layer of its own (`pdfHasTextLayer` is `false`), renders the pages with `pdftoppm -r 300 -png -gray` in chunks of 20 and runs ONE `tesseract <list> <base> -l <lang> pdf tsv`, which writes the searchable PDF and the per-word TSV in a single pass. Stores `searchablePdfPath` beside the original, the engine, the two confidence figures, and re-derives `rawText`/`rawWords` from the copy. `lib/ocr-tesseract.ts` and `lib/ocr-surya.ts` are the two engines behind one `OcrRunner` contract, `lib/ocr-text-layer.ts` the step, `lib/tesseract-languages.ts` the ISO-639-1 → tessdata table, `lib/tessdata.ts` the managed directory and the language packs: `packages/server/src/lib/tessdata-manifest.ts` is generated by `scripts/tessdata-manifest.mjs` from the `tessdata_best` commit pinned in `scripts/pins.json` (name, bytes and git blob SHA-1 per pack — sizes are never typed by hand), a pack downloads as one file verified against that SHA-1 and renamed into place, and tRPC `ocrLanguages` lists/downloads/removes them for `SettingsModal` and the `<OcrLanguagePackRow>` that `ExtractModal` shows when the book's language has no pack yet. A missing pack fails the OCR step by name rather than reading the page in the wrong language. **Surya** (`books.ocrEngine = "surya"`) runs `scripts/ocr_surya.py` in the Python env — detection + recognition per page, then an invisible text layer written onto a copy of the original with `pypdf` (BSD-3), one string per line in a glyphless font (`scripts/glyphless.ttf`, tesseract's `pdf.ttf`, Apache-2.0) drawn in render mode 3 and scaled to the line's box, exactly Tesseract's technique, so `pdftotext`, pdf.js and `page_geometry.py` see the words where the ink is; `/Rotate` pages are handled by writing in user space with a rotated text matrix. The script speaks JSON events on stdout (`start`, `page`, `detected`, `line`, `page-done`, `done`; boxes in PDF points of the displayed page, origin top-left) and `--page N --stream-lines` recognises one page in chunks of four lines so a screen can show lines as they land; `lib/ocr-surya.ts` parses them, needs the `extraction` model bundle, tries MPS/CUDA then CPU like marker, and reports no confidence figure — Surya's is not comparable to Tesseract's per-word one, and `return_words` was measured to hand every word its line's box, so line-level is all it can place. Above `OCR_GARBLED_FRACTION` (15% of words under Tesseract's confidence bar) `books.get` marks the file `ocrGarbled` and the Source files tab offers **Redo with Surya**, which is `books.retry` with `forgetTextLayer: true` — the searchable copies are unlinked and the OCR columns cleared before the usual full re-extract, so the extract worker reads the pages again with the new engine. Queued after every raw-only upload and append (a no-op when every file carries text, so a born-digital book never notices); a `fullExtract` upload lets the extract worker call `ensureTextLayer` inline per file so the pages are read once. **The default is decided, not asked:** with `books.ocrEngine` null a text-less file is read with Tesseract and that choice is then written to the book, so a scan uploaded with extraction on simply works; the language is `books.language` when set, else the page's script from Tesseract's OSD (`--psm 0` on a 300 dpi render of a middle page, trusted only above confidence 1 — at 150 dpi it called an English page Cyrillic) mapped through `packsForScript` in `lib/tesseract-languages.ts` to the first installed pack, and a script with no installed pack fails naming the packs to download rather than reading Cyrillic as English. Registers its abort controller under the bookId, so `books.cancel` stops it exactly as it stops marker, and a cancelled run leaves no searchable copy. Re-running is a no-op unless `force` or the copy was written by the other engine — changing `books.ocrEngine` and re-extracting reads the pages again with the new one — and a file that carries a text layer of its own is never re-read. `books.retry` and `extractChapters` refuse while an extraction is in flight (`extractRunning` in `lib/extract-registry.ts`), and every `extract` enqueue carries `jobKey: extract:<bookId>` with `jobKeyMode: "replace"`, because two runs on one book inserted every chapter twice. **The third engine is a cloud vision model** (`books.ocrEngine = "llm"`, `lib/ocr-llm.ts`, model in `books.ocrModel` or the Settings default): each page is rendered at 1600 px on the long edge (DeepSeek resamples anything larger, so more pixels only cost upload bytes), encoded WebP when `cwebp` is on the PATH and JPEG otherwise, and sent with a fixed prompt through the AI SDK's structured output (`Output.object` on a loose Zod shape — the model writes `text` for `paragraph` and `content` for `text`, and a strict enum failed half the POC book's pages). The pages become a Marker-compatible layout JSON at the top of the file's outDir (`llm-layout.json` + `_meta`, which `findMarkerJson` picks before any Marker subdirectory) plus `rawText` on the row with `\f` between pages — the model's own text, not pdftotext's — and the extract worker passes `prebuiltLayout` so `extractPdf` skips Marker and detects chapters from that JSON; every other engine removes a stale `llm-layout.json` before Marker runs. **Positions come from a local read:** on macOS Apple's Vision recogniser (`scripts/vision-words.swift`, a Swift binary `lib/ocr-vision.ts` compiles on first use in development and `desktop-build.sh` places beside the bundled tools) boxes every word, following skewed and clipped lines Tesseract's layout analysis drops whole — Bulgarian is read in Russian mode with correction off, so the letters come back and the fuzzy alignment forgives the spelling; elsewhere Tesseract runs as `tsv` (`parseTsv` in `lib/ocr-tesseract.ts`). Tesseract in the book's own language stays the text the exact-word fidelity check compares against, Vision serves both only in a language it has natively (`combineReadings`, `makeReference`); when both read a page, both placements are tried and the one that matched more words is kept (Tesseract won 7 of the POC book's 19 pages), and a Vision failure at runtime is logged once and costs the boxes, never the paid run. Matched words carry their similarity, and those under `DOUBT` (0.85) — the readers spelling a word differently, where the model's own misreads hide — are named per page in the log and kept in `llm-layout_meta.json`. `lib/word-alignment.ts` aligns the model's words to the boxes — Needleman–Wunsch over the two sequences with Levenshtein-fuzzy equality at 0.7, one model word allowed to take two OCR words when the first ends in a hyphen, words the reader missed placed between their placed neighbours, else at the start of the next placed word's line (a skipped line beginning is the common case) or after the previous one, squeezed to stay on the page — a run that hung off the right edge once made the viewer, which sizes a page to its content box, draw the page at a quarter width; OCR words the model dropped (bleed-through, furniture) ignored. The order constraint is what tells one `на` from the next. Scaled by `pdfPageSizes` (pdfinfo, sides swapped for `/Rotate` 90/270 — pdftoppm and pdfium render the rotated view) the boxes become block `polygon`s in the layout and one invisible string per word — on the middle `LAYER_BAND` of its box, since Vision's full-height boxes touch across rows on a skewed page and `page_geometry.py` then reads two rows as one line — in a searchable copy written by `scripts/pdf_text_layer.py` (`lib/pdf-text-layer.ts`), which reuses `ocr_surya.py`'s glyphless-font writer, so `page_geometry.py`, pdf.js search and the read-along see the AI's words where the ink is — per word, which is finer than Surya's per-line placement. The copy is a bonus on the paid reading: a writer failure is logged and the book goes on without in-PDF search. With no Tesseract pack for the language there is no reference, no placement and no copy, and `textLayerDone` is the one place that knows a file read that way is still done. `llm-layout_meta.json` carries the mean placed share; the per-page log line carries each page's. **The reading is kept apart from the placement:** `llm-pages.json` in the outDir holds the model's pages as read — written as pages land (`complete: false` until the end), so a run that fails or is cancelled keeps what it paid for and the next run reads only the missing pages — and `books.replaceWords` (`replaceFileWords` in `lib/ocr-text-layer.ts` over `replaceWords` in `lib/ocr-llm.ts`) places them again with the current readers and alignment — no model call — rewriting the layout and the copy and refreshing the chapters' block polygons in place by page and text (`matchBlockPolygons`), so a better placement never costs a re-read or a re-synthesis. `scripts/capture-ocr-fixture.ts` captures exactly what that chain consumes for a real book — the pages as read, page sizes, both readers' word tables — into `test/fixtures/private/<name>/` (gitignored: published books), and `ocr-llm-bg-scan.test.ts` runs the whole chain on it, blank pages of the same sizes standing in for the scans, asserting placed share, boxes inside the page, the skipped-line-start case, per-page joins and the share of sentences the reader can put on lines; it skips when the fixture is absent. A page-end continuation is joined only when a hyphen split the word (`joinContinuations`): a block glued across pages carries one page and one polygon, and every cue in its second half was drawn on the first page's box; the reader spans a sentence over two blocks itself. Whatever the engine, `ensureTextLayer` removes the file's `geometry.json` sidecar when it writes, replaces or drops a copy (`removeSourceGeometry` in `lib/page-geometry.ts`): the sidecar describes one text layer, and one built before OCR — a book opened in the reader first — has no lines and made every cue fall back to its block's polygon until it was rebuilt. Fidelity is measured per page against Tesseract in the book's language (no pack → no check, logged): word-multiset recall over the reference words in the model text's dominant script, because Surya read a page's bleed-through as English and Chinese; under `RETRY_BELOW_RECALL` (0.88) the page is asked once more with the local OCR's first and last lines as anchors — the model deterministically skipped a page's opening fragment and no prompt wording stopped it, the anchors did — and pages still short are logged by number. A parse failure is retried up to three times; any other provider error aborts the whole run, because retrying into a bill is worse than a failed extraction. `ocrConfidence` holds the mean recall and `ocrLowConfidenceFraction` the share of flagged pages; `isGarbled` stays Tesseract-only. Soft hyphens (U+00AD) are stripped: the model "joins" a line-end hyphen by writing one. Measured on the 19-page Bulgarian typescript that prompted it: 95% recall against Surya, 93% against Tesseract `bul`, 81–86 s, 2.3 cents; `scripts/llm-extract-poc.ts` re-runs that measurement on any PDF.

1. **extract** (`workers/extract.ts`) — Runs `marker_single` (Python subprocess) on the searchable copy where there is one, outputs structured JSON into a subdirectory. Flattens ALL blocks (not just kept types) with page numbers, polygon coordinates, and an `included` flag. **Chapter detection**: if enabled, first attempts LLM TOC-guided detection (`lib/toc-detect.ts`, model from `books.chapterModel`, default when null): reads the printed TOC from layout-preserving `pdftotext -layout` text (indentation = hierarchy, entries carry a `level`), asks the model which TOC tier is the chapter tier, maps printed page numbers to PDF pages from every strong title match (`lib/toc-anchor.ts`, a drift-tolerant monotonic chain keeping one anchor per entry, so a chapter heading repeated in the notes or index loses to the body offset), and places each chosen entry on the heading near its expected page, asking the model only about the garbled ones. Without a usable TOC it selects from the heading catalog (with words-after-heading). The parsed TOC is persisted on the proposal (`chapterProposal.toc`) so the structure modal can show what was found. Falls back to the numbered-chapter tier (Chapter N / Глава N sequences, ToC listing pages excluded), then the heading-level heuristic (h1 → h2 → fallback word-count split). Stores per-chapter `sourceBlocks` (jsonb) with full block metadata, plus `pageStart`/`pageEnd`. Creates chapter rows in DB. Queues normalize jobs. When the upload asked for auto-synthesis (`skipSynthesis: false` — the unattended "drop a PDF, get an audiobook" path) it also queues a `waitForAll` **assemble** here, so the promised M4B is a visible waiting job rather than something inferred later.
...
Workers run in seven pools, declared in `workers/pools.ts` and started by `workers/setup.ts`: `tts` (default concurrency 2 — MLX contends for the GPU), `raw` (2 — rawExtract, so raw text never queues behind a 30-minute marker run), `extraction` (1 — extract/redetect/propose/ocrTextLayer), `prep` (2 — normalize, milliseconds of regex that must never queue behind a marker or propose run), `assembly` (1 — assemble/assembleDocument, separate so exports never queue behind a long extraction), `translate` (3 — translate/translateTitles/cleanup/bookNote/digest), `index` (1 — indexBook/embedChunks; BGE-M3 contends for the GPU with TTS). Each default is overridable from Settings → "Background work", which writes `WORKER_CONCURRENCY_<POOL>` to `.env` and rebuilds that one runner (`applyPoolConcurrency`) — graphile fixes concurrency at runner construction, and its graceful shutdown kills anything still running after five seconds, so the mutation refuses, and the UI disables the control, while that pool has a locked job. `POOL_META` owns the per-pool ceiling and the caution copy; `tasks` there is what maps a locked job back to its pool. Every `synthesize`/`synthesizeTranslation` enqueue takes its spec from `synthesisJobSpec` (`lib/synthesis-jobs.ts`): a voice on one of the MLX narrators (`bg-mlx`, `kugel`) puts the job in the graphile queue `tts-mlx`, which runs one job at a time while workers walk past it to anything else. Those narrators share one model behind `runExclusiveMlxSynthesis`, and unnamed their jobs each took a `tts` slot just to wait on that lock — with a first-in-first-out queue, a Bulgarian book held both slots and a Kokoro book queued behind it showed 57 chapters "synthesizing" with nothing running. Raising the pool's concurrency does not fix that; the next slot takes the next Bulgarian job. All jobs use `maxAttempts: 1` — no silent retries. Assemblies and document exports are deduplicated via graphile `jobKey` (`lib/output-readiness.ts` owns the key format, so repeat requests and the upload path's queued assembly collapse into one job), and queued/running/waiting exports are surfaced by `books.pendingDocumentExports`.
...
| **pdftotext** (poppler, system binary) | Fast raw text extraction at upload; `pdfHasTextLayer` is the probe the OCR step keys off | `lib/pdf-raw-text.ts` |
| **Surya OCR** (already in the env for marker; `pypdf` writes the layer) | Reads a scanned PDF into a searchable copy with line-level boxes — slower, better on photographed, curled or faded pages | `scripts/ocr_surya.py`, called by `lib/ocr-surya.ts` |
| **AI OCR** (cloud; any configured LLM provider with image input) | Reads a scanned PDF page by page into a Marker-compatible layout — joins split words, drops furniture, any language the model reads; word positions from a local Tesseract read aligned to the model's text, written into the searchable copy | `lib/ocr-llm.ts`, `lib/word-alignment.ts`, `scripts/pdf_text_layer.py`, through `lib/llm.ts` |
| **Tesseract 5** + **pdftoppm** (poppler; `brew install tesseract`, apt `tesseract-ocr tesseract-ocr-eng tesseract-ocr-osd`) | Reads a scanned PDF into a searchable copy with word-level boxes. `TESSDATA_PREFIX` is **one** directory and needs `configs/pdf` + `pdf.ttf` beside the `.traineddata`, or `tesseract … pdf` fails instantly with no useful message | `lib/ocr-tesseract.ts` |
...
**books** — id (uuid), title, kind (`pdf` | `digest` | `api`, default pdf), filename + pdfPath (nullable — null for synthetic books), outputPath, status (`pending` | `extracting` | `synthesizing` | `assembling` | `done` | `failed` | `suspended`), voice, speed, error, ocrEngine (`tesseract` | `surya` | `llm` | null — null means the pages carry their own text and no OCR step runs), ocrModel (nullable — the vision model the `llm` engine reads with, a registry key; null = Settings default), llmChapterDetection, chapterDetection, chapterProposal (jsonb), translationLanguage (last active variant key — drives the variant view default), variantVoices (jsonb `VariantVoices` — per-variant-key voice/speed overrides keyed by lane; absent fields fall back to books.voice/speed, set via `variants.setVoice`), skipSynthesis, totalChapters, structureConfirmedAt (set by `books.applyChapterBoundaries` — applying is the review — or by `books.confirmStructure` when the checked boundaries are the chapters as they stand, so nothing is re-sliced; cleared by `resetChaptersKeepingInserted`, so every rebuild asks again — the chapter tray's one orange button follows it: Review chapters, pulsing, until confirmed (a book with no PDF skips this stage — its chapters arrived as they are, and there is no structure to review); then Synthesize; then Export once selected chapters have audio and none is in flight), noteJob (jsonb), origin (jsonb `BookOrigin` — digest or api provenance), digestJob (jsonb `DigestJob`), folderId (FK folders, `set null` on folder delete — book deletion must go through `lib/delete-book.ts` for disk cleanup, so never cascade), profileId (FK profiles, defaults to the fixed default-profile id), createdAt, updatedAt
...
**notes** — id (uuid), bookId (FK, cascade delete, **nullable** — null marks a library-chat answer), profileId (FK profiles), prompt, model (a model key — a pinned `flash`, a discovered `deepseek:…`, a local `ollama:…`; a saved legacy `pro` resolves to the model it named), result (markdown), scope (jsonb `NoteScope` — chapter id+title snapshot, `book-raw`, or `library {folderId?, question}`; no FK to chapters so notes survive chapter re-detection), createdAt. Auto-inserted by `chapters.aiPrompt`, `books.aiPromptRaw`, the `bookNote` worker, and `notes.saveLibraryAnswer` via `lib/notes.ts` `saveNote()`. `notes.toChapter` refuses library notes (no book to attach to).

**chat_conversations** — id (uuid), profileId (FK profiles, cascade), title (empty until the first question names it — an 80-character excerpt, no model call), kind (`library` | `assistant` — every conversation is `assistant` since migration 0044 converted the library chat's; `library` stays in the union for the retired `POST /chat` route and its tests), scope (jsonb `ChatScope`, fixed at creation; book titles and the folder name are snapshots so a removed source can still be named), citedBooks (jsonb `ChatBookRef[]` — books the answers quoted), model (last model key used), createdAt, updatedAt (last activity; the sidebar's order). No FK to books or folders on purpose: a conversation outlives what it searched.

**staged_files** — id (uuid), profileId (FK profiles, cascade), conversationId (FK chat_conversations, cascade, nullable until a thread sends it), filename, sizeBytes, sha256, path (`dataPath`), status (`uploading` | `ready` | `used` | `removed` | `expired` — the row outlives the file by a day so a chip can say why it is gone), createdAt, lastActivityAt (the expiry clock). See `POST /upload/staged`.
...
packages/server/src/
  ...
  routes/
    ...
    chats.ts            Library-chat conversations: list/get/create/rename/delete, book options for the source picker
  workers/
  ...
  lib/
    ...
    toc-detect.ts       LLM TOC-guided chapter detection (prompts + orchestration)
    toc-anchor.ts       Pure matching for it: title similarity, printed→PDF page map, entry placement
    chapter-rules.ts    Preface/oversized/label-sized word thresholds, shared with the structure modal
    ...
    chats.ts            Conversation store for library chat: scope resolution, run registry, question/answer persistence
    ...
    mcp-server.ts       The MCP tool set: ~20 curated tools over appRouter.createCaller, plus wait_for_book polling
    mcp-library.ts      Profiles and folders by name for the MCP tools: resolveProfileId, resolveFolderId (path from the top level), folder paths with counts
    voice-catalog.ts    The static voice table (Kokoro, narrators) and runtime-voice converters; dependency-free, the web picker imports it
    voice-list.ts       Every usable voice in one flat list: catalog + installed macOS/Pocket + cloud libraries behind a key (MCP list_voices)
...
packages/web/src/
  ...
  lib/
    ...
    row-click.ts        rowClick(action): a table row's main action on a click anywhere in it — the chapter
                        row opens the chapter, the source-file row previews the PDF. Controls inside
                        the row keep their own clicks, and a click that ends a text selection is ignored
  ...
  pages/
    ...
    Reader.tsx          Read-along reader (/books/:id/read): Column/Page/Text views over one timeline,
                        phone width presets + legibility readout, tap-to-seek, Back inside the pinned bar (a chapter is
                        thousands of pixels of pages), `?t=<ms>` to open at a moment of the narration, rect/layout debug toggles.
  ...
  components/
    ...
    UploadZone.tsx      Drag-and-drop PDF upload; one book or many, a title, nothing else — it lands on the book with Extract open. Lives
...
**books** (pipeline): `upload` (legacy tRPC path) · `retry` (full re-extract; `forgetTextLayer` drops the OCR copies first) · `extractChapters` · `processSelected` · `cancel` · `assemble` · `assemblies` / `deleteAssembly` · `structure` / `proposeChapters` / `applyChapterBoundaries` / `redetectChapters` · `chapterList` · `rawTextStats` (Ask AI itself is the assistant's `analyze_text` tool)
...
**ocrTry**: `page` (renders one page of a file at 150 dpi under `data/tmp/{bookId}/ocr-try/`, plus a 300 dpi copy for OSD; returns page count, size in points, image URL, detected script, its candidate packs, installed packs, whether the Surya bundle is present) · `tesseract` (one page through `tesseract … tsv`: lines with per-word confidence and boxes in points, mean confidence, doubted share, and the callout — `clean` / `edge` with side and band / `scattered`, from `classifyDoubt` in `lib/ocr-try.ts`)

**ocrLanguages**: `list` (every pack in the pinned tessdata_best manifest — name, bytes, book language, installed, in-flight download) · `download` (one file, checksummed, atomic) · `remove` (refuses `eng`)

**chats**: `list` (profile's conversations newest first — scope and cited books with current titles and `available`, every question asked for the sidebar search, answer count; never-asked ones hidden) · `get` (scope resolved against today's library, messages as opaque parts + `{status, error, modelKey, modelLabel, createdAt}`, plus the live partial and `running` while an answer is being written) · `bookOptions` (id, title, author of every book in the profile) · `create` (scope validated against the profile, titles snapshotted) · `rename` · `stop` (the only way to end an answer early; returns once the run has let go) · `delete` (ownership checked before the run is touched, then aborts a running answer; books and saved notes untouched) — all profile-scoped
...
- `GET /ocr/try/:bookId/:fileIndex/:page.png` — the rendered page the Try one page screen shows; `GET /ocr/try/:bookId/:fileIndex/:page/surya` — SSE, one page through `scripts/ocr_surya.py --page N --stream-lines`, the script's JSON events forwarded verbatim plus `{event:"error"}`; closing the stream SIGKILLs the run. Both registered by `lib/ocr-try-routes.ts`; the Vite proxy forwards `/ocr`.
...
- `POST /mcp` (also GET/DELETE, answered per the MCP spec) — MCP server over Streamable HTTP (`mcp-routes.ts` + `lib/mcp-server.ts`, tool reference in `docs/mcp.md`). Stateless: each request builds a `McpServer` and transport and tears them down on response close, so an app restart strands no session. Tools call the tRPC procedures in-process through `appRouter.createCaller({ profileId })` — the profile comes from the same `x-profile-id` header as everywhere else — so validation and behaviour are the router's, never a second copy; `upload_book` is the one tool with its own body, because it takes local paths (copied into the upload dir, then `lib/pdf-books.ts` like the multipart route). Every long job returns at once with a *summary* (`summarizeBook`: counts, and in detail only what failed — `get_book` is the one tool that lists chapters, because an agent pays per token and polls in loops); `wait_for_book` polls `books.get` every 2s until a stage (`text`/`searchable`/`chapters`/`audio`/`output`) or its timeout, returning early on failure. `text` means every file has been read, OCR included — it reads the job queue (`queuedWork`) as well as the book, because between a raw extraction ending and the OCR job starting the book looks idle, and one file with text used to satisfy it for a seventeen-file book while the first scan was on page 11. The profile header is fixed when a client is configured, so the tools that place or list things also take `profile` and `folder` by name (`lib/mcp-library.ts`; a folder is a path from the top level, created when a book is being placed and never for a filter). `create_book` is `lib/api-books.ts` behind a tool — text in, searchable and narratable book out — and `save_note`/`list_notes` are how an agent leaves what it worked out where the person and the next session will find it. No Origin exists to judge here, so `isTrustedHost` (loopback, any IP literal, or `TRUSTED_HOSTS`) is the whole rebinding check. Keep the tool count near 20 (24 today — `manage_folder` creates, renames and moves folders and never deletes; the log folded into `get_book` to make room) — agents degrade past that — and add one only with a description that says when to use it. `get_capabilities` + `start_download` exist because the UI's bundle/pack gates (`useModelBundle`, the OCR pack picker) do not exist on MCP: `upload_book`/`extract_book` refuse full extraction until the `extraction` bundle is installed and name the download instead of failing offline in Python.
- `POST /assistant` — The assistant panel's answers (`assistant-routes.ts`): the same stored conversations and server-owned run as `/chat` (`lib/chat-run.ts` `deliverAnswer` is the shared half), with the MCP tool set in place of the search tools — `lib/assistant-tools.ts` connects `createMcpServer` over an in-memory transport and hands the model the tools whose tier it is allowed (`TOOL_TIERS`: read / quick / confirm / spend; a test fails on a tool the table does not name), and which calls wait for a yes is `needsApproval`, the SDK's `toolApproval` hook, decided per call (`tierOf` in `lib/assistant-tiers.ts`, dependency-free so the web panel draws a call as a trace, a Done card or a confirm card from the same table): read and quick tiers run in the turn, confirm and spend end the turn as an `approval-requested` part the panel shows as a card. Run or Cancel comes back as `trigger: "approval"`; the server marks the stored part responded and generates again from the same message (`deliverAnswer`'s `continueFrom` — the stream carries that row's id, the SDK executes the approved call itself, and the kept copy replaces the row), so a card and what followed it are one answer. A card walked past is answered no on the person's behalf before the next question. Quick tools answer with the call that reverses them (`undo` in the result: a rename, a move, an author, a folder rename or move) and `POST /assistant/undo` runs it as the server, no model turn. Message ids are the stream's, generated by the server, so the browser, the stream and the row agree. `show_in_app` is the one tool the panel has and `/mcp` does not (an agent has no screen): a static AI SDK tool in `lib/assistant-tools.ts` whose input is a place in the app — the library, a folder, a book tab, one of the book page's four dialogs, a chapter, the reader at a moment — turned into a URL by the dependency-free `lib/assistant-targets.ts`; the panel follows the call as it lands (`navigate`, once per call, never for a restored transcript) and draws it as an "Opened …" line. The book page opens a dialog from `?dialog=extract|structure|synthesize|export` (consumed on arrival, `?extract=1` kept as the old spelling) so the panel can open one under a page that is already mounted. After a reload a card still waiting is checked against the book: one whose `updatedAt` is later than the card's message is drawn "Out of date" with no buttons. A thread being answered by a run this window did not start is polled (`chats.get` every second while `running`) and the transcript is the server's until it ends, when the thread remounts from what was kept. A stream that ended with neither words nor a call is kept as failed, so the person sees an ending and a Retry rather than a blank. The panel also has the library chat's search: `assistantTools` takes the chat's `search_library` and `read_passage` (built by `buildChatTools` with a `CitationCatalog`, scoped to the book on screen or the profile) in place of the MCP `search_library`, the route verifies the answer's `[c_N]` ids and writes `data-sources` exactly as `/chat` does, and the thread renders them with the chat's `SourceList` — so a "where does it say" question in the panel gets links into the page or the reader. The model's reasoning is shown as a collapsed "Thinking…" line (the `thinking-sweep` keyframes in `styles.css` while it streams). Several dropped files carry a one-or-many choice (`BundleChoice`, sent as a sentence in the question) and can be reordered; a dropped folder is walked for PDFs by `droppedPdfs` in `lib/dnd.ts`, shared with the upload zone. The body carries one question, the page (`screen`: route and book id, summarised into the system prompt by `lib/assistant.ts`) and any `staged:` references. Conversations carry `kind` (`library` | `assistant`); each route refuses the other's. Since 2026-09-25 the panel is the one chat: the `/chat` page and the "Chat" links on the library and book headers are gone (the Assistant toggle beside them is the way in; the book's search-index dot moved onto the panel's sources chip, `lib/search-index-state.ts`), past threads and the "Saved answers" list are behind the panel's History button (`HistoryModal`), every conversation is `kind: "assistant"` (0043 sets the default, 0044 converted the rest), and Ask AI's presets are chips on a book page that become `analyze_text` cards (`lib/assistant-tools.ts`: `buildAskContext` → `generateText` → `saveNote`, the answer drawn as a note with "Add as chapter"). A thread's sources are chosen once (`SourcesModal`: follow the page, or the chat's `SourcePicker`; the header chip names what a question would search right now — the book on screen or "Whole library" — never the rule, because "Follows the page" read as a mystery) and a new thread started inside one carries them minus what is gone (`draftFromScope`); one whose sources are all gone is read, not continued (`askable` → `OpenThread.closed`). "Save as note" under a finished answer is `notes.saveLibraryAnswer`, as on the old page.
- `POST /upload/staged`, `DELETE /upload/staged/:id` — A PDF dropped on the assistant panel (`staged-routes.ts`, `lib/staged-files.ts`): streamed to `data/tmp/staged/<profile>/<id>.pdf`, hashed, refused unless it starts `%PDF-`, capped at 2 GB per profile (uploads run one at a time per profile, because the cap is a read of the rows plus the stream in hand and files dropped together would each pass it alone), answered with `staged:<id>`. `inspect_pdf` and `upload_book` take that reference in place of a path (the model never sees a path); `upload_book` copies the file into the book and marks the row `used`. A file is claimed by the thread that first sends it (deleting the thread deletes it), expires a day after that thread's last use, and `sweepStaged` (at boot and hourly) settles expired rows, files with no row and rows with no file.
...
- `POST /chat` — Retired from the web app on 2026-09-25 (the assistant panel is the one chat and answers over `/assistant`) and kept with its tests as the search half's own harness: the library-chat streaming endpoint (`chat-routes.ts`). Raw Fastify route because tRPC can't stream; AI SDK UI-message stream over `reply.hijack()` + `pipeUIMessageStreamToResponse`. Body is `{conversationId, trigger, text, model}`; the scope and the history come from the stored conversation, never from the request. Profile via `x-profile-id`. 404 unknown conversation, 409 already answering or nothing left to search. Closing the connection does not stop the answer — `chats.stop` does.
- Ask AI has no route of its own any more: the book page's Ask AI buttons (header, the raw-text hint, the chapter tray, a chapter row, the chapter modal) pin the book or the chapters to the assistant (`pin` in `components/assistant/context.tsx`), the pinned text rides in the question as `Read the whole text of "…" (bookId …).` or `Read these chapters of "…": … (chapter …).` (`describeRead` in `assistant-routes.ts`, titles from the library, refused when gone), and the model answers with the `analyze_text` spend tool — `lib/ask-ai.ts` `buildAskContext` behind a card, the answer saved as a note and drawn as one. `POST /chat/ask` and `ChapterAiModal` were removed on 2026-09-25.
...
- **`scripts/bundle-tools.py`** — copies ffmpeg, poppler (pdftotext/pdfinfo/pdftoppm) and tesseract
  out of Homebrew with their dylib closure, rewrites load commands to `@loader_path`, and
  **re-signs ad-hoc**, without which Apple Silicon kills them silently. It also copies
...
Agentic RAG over the whole library (the assistant panel, docked beside every page): the picked model iteratively calls search tools over an indexed copy of every book's text, streams a cited answer, and each citation opens the source (PDF page / chapter / translation view). Scope: whole profile, a folder subtree, or a single book ("💬 Chat" on BookDetail → `/chat?bookId=…`). Conversations are kept (`chat_conversations` / `chat_messages`, see **Conversations** below) and listed in a history sidebar; an answer worth keeping on its own is still saved explicitly as a note (`notes.saveLibraryAnswer`, `bookId NULL`) and listed behind the sidebar's "Saved answers" button via `notes.listLibrary` — deleting a conversation never touches those. This is distinct from **Ask AI** (the assistant's `analyze_text` tool, reached from the book page's Ask AI buttons or the preset chips) which stuffs the full scope text into context with no retrieval — better for whole-book analysis, no citations.
...
**Chat loop** (`chat-routes.ts` + `lib/chat-tools.ts` + `lib/citations.ts`): Vercel AI SDK `streamText` with the model resolved from `lib/llm.ts` (any configured provider; models without tool support are rejected for chat), `stopWhen: stepCountIs(8)`, 3-min abort signal, 4096 max output tokens, and a `prepareStep` that on the last step removes the tools (`activeTools: []`) and appends a user message saying the search rounds are spent and the answer is due now, so the turn always ends with a text answer instead of closing silently after tool calls. It first declined them with `toolChoice: "none"`, and DeepSeek answered that by writing the calls it still wanted as raw DSML markup — the tools have to be absent from the request. Tools: `search_library` (hybrid search, registers hits in a per-request `CitationCatalog` as `c_1…` ids), `read_passage` (neighbor expansion by citation id), `list_books` (titles only). The model must cite `[c_N]` inline; after streaming, `verifySources()` keeps only catalog-known ids (toc-detect discipline — hallucinated ids are dropped) and emits one `data-sources` part the UI renders as a source list; on the way out `withReaderTargets` (`lib/citation-targets.ts`) gives each cited passage of a narrated chapter a `readAt` — the chunk's `charStart` located among the sync map's cues with `locateChunks`, null when the narration was made from a text that no longer matches. The system prompt is built per request (`libraryChatSystem`): `scopeLanguages` reads `books.language` for the scope, and the model is told to write its queries in that language — one language means every query in it, several means English first and another only when the question plainly concerns such a book or English found nothing. It used to be told the library "mixes English and Bulgarian" and ran every search twice. The catalog is re-seeded from the stored history's `data-sources` parts so follow-up turns can cite earlier ids.

**Conversations** (`lib/chats.ts`, `routes/chats.ts`, `chat-routes.ts`): the transcript is the server's. `POST /chat` takes `{conversationId, trigger: "submit" | "regenerate", text, model}` — one question, never a history to be believed — verifies the conversation belongs to the profile, saves the question **before** generation, and loads the history from Postgres. A conversation's scope (`ChatScope`: library, folder, or a chosen set of books with title snapshots) is fixed at `chats.create`, which the page calls on the first question; changing sources means another chat, so an earlier answer can never appear to have searched a later selection. `searchScopeOf` turns it into what the tools may search *today* and returns null when everything chosen is gone — the route answers 409 and the composer closes; **a missing scope is never widened to the library**. **The run is the server's, not the tab's.** The response stream is teed: one branch goes to the browser, the other is read with `readUIMessageStream` and kept by `saveAnswer` with how it ended — `complete`, `stopped` or `failed` — so a partial answer survives. Closing the page ends the delivery and nothing else: the answer is finished and kept for whoever opens the conversation next. It was first built the other way round — a dropped connection *was* Stop — and a question asked from a tab that was then closed came back "Stopped before it said anything". The only things that end a run early are `chats.stop`, deleting the conversation, and the 3-minute timeout. `chats.stop` aborts and **awaits the run's end** (`stopChatRun`), so "Ask again" straight after is never refused as already answering; the run is released when the *kept* copy ends, never on `pipeUIMessageStreamToResponse`, which does not settle once the browser's response is closed — releasing on it left every stopped conversation answering 409 until a restart. While a run is going its latest partial sits in the registry (`liveAnswer`), `chats.get` appends it with `status: "streaming"` and `running: true`, and a window that did not start the run polls that once a second (`refetchInterval` in `pages/Chat.tsx`) and shows it; when `running` clears the pane is remounted and seeded from what was kept. Nothing is ever retried on its own. "Ask again" is `trigger: "regenerate"` **addressed by the question's ordinal** (`question`: the page's count of questions up to the one retried, plus its `text`) and resolved by `retryQuestion`: a question refused before it was saved — no model, no tools, already answering — exists only in the page, is one past what is stored, and is saved then as the submission it never got to be; a saved one has whatever followed it dropped; any other count is refused as out of step. Unaddressed, a refused second question regenerated the *first*: its answer was deleted and the new one filed under the wrong question. An answer that did not finish still gets its `data-sources` (verified from the partial text against the run's catalog) before it is kept — the stream only carries them on a clean finish, and without them every `[c_N]` already written rendered as nothing, for good. `chats.get` returns `removedBookIds` for every book the transcript cites or chose that is gone, whatever the scope: a whole-library chat cites books it never listed as sources. The response can end a moment before the answer is kept, so anything that reads the table right after — `chat-routes.test.ts` does — waits for the run to be released. `messagesForModel` replays a cut-short answer's words without its tool calls, because a call with no result is not a valid turn. One run per conversation (`claimChatRun`, in memory like the extract registry → 409); `deleteConversation` aborts the run before deleting the row, and `saveAnswer` looks the conversation up first, so a late answer cannot bring it back. A history that outgrows the model's context is refused by name — nothing is dropped or summarised to make room. A conversation with no title was never asked anything (its first question was refused): `chats.list` hides it and `createConversation` sweeps day-old ones. `citedBooks` accumulates the books answers actually quoted, which is what lets the sidebar's book filter offer "Quoted it" beside the default "Chose this book" — a book merely being searchable never counts.

**Frontend** (`components/assistant/`, `components/chat/`): the assistant panel is the chat — `AssistantPanel.tsx` holds the header (sources chip, History, New chat, Collapse), `AssistantThread.tsx` the transcript and composer. There is no chat page: the panel remembers its last thread per profile (`lib/assistant-prefs.ts`), the History dialog (`ChatSidebar.tsx` inside a `Modal`, with the search, the book filter and the Saved answers list) opens any other. The thread is keyed by a counter, not the id: one born in it gains an id mid-answer, and remounting there would drop the stream. `useChat` + `DefaultChatTransport` with `prepareSendMessagesRequest` sending only the new question; the conversation id, the model, the page, any staged files and any pinned text ride in each request's body. `chats.get` is read once per opening (`gcTime: 0`) because the thread seeds `useChat` from it and then owns the transcript. Message helpers (`messageText`, `messageSources`, `renderText`) are `lib/chat-message.ts`. History filtering, grouping and the scope summaries are pure in `lib/chat-history.ts`; sidebar filters are form fields in the sidebar and a chat's sources are chips in its own header, so the two never read as the same control. `SourcePicker.tsx` shows a capped list with a type-to-narrow field (title or author) — libraries run to hundreds of books. **New chat pressed inside a conversation starts on that conversation's sources** (`draftFromScope`, minus anything since removed), with the picker open and the carried books ticked first: re-picking books for every chat was a chore, while widening is one click ("clear", or Whole library). Tool calls rendered as collapsed "Searched: …" lines, `[c_N]` markers rewritten to `[n]` numbering matching the source list (`SourceList.tsx`: grouped under the book's title, each row the chapter and page plus how the passage starts — a dozen citations of one chapter were a dozen identical truncated pills). Row targets: a narrated chapter → the reader at the cue, `/books/:id/read?chapter=<index>&t=<ms>`, with the PDF kept beside it as a second button; otherwise raw or chapter with a resolvable file+page → `PdfPreviewModal` (`/pdf/:fileId#page=N`), else `/books/:bookId`; translation/variant → `/books/:bookId?variant=<key>`. `?t=` is where an unplayed chapter starts in `Reader.tsx` — the follow-scroll lands on whatever cue holds that moment.
...
1. **LLM-based detection** (when enabled): TOC-guided selection via `lib/toc-detect.ts` (prompts, orchestration) and `lib/toc-anchor.ts` (pure matching). Reads the printed TOC from `pdftotext -layout` text with indentation levels, asks the model which TOC tier is the chapter tier, maps printed page numbers to PDF pages from confident title matches, then places each chosen entry on the heading near its expected page and asks the model only about garbled ones. Without a usable TOC it selects from the heading catalog (selecting ~all candidates is treated as failure). Reasoning is off on every call — these are structured lookups. The structure modal flags the result against `lib/chapter-rules.ts` (oversized and label-sized chapters) rather than asking the model to review itself. Falls through if the API call fails or returns <2 chapters.
...
**Propose (LLM) button** (structure modal) uses `workers/propose.ts` → `lib/toc-detect.ts` with the selected `books.chapterModel`. All source PDFs in book order form one continuous book, with global page/block positions mapped back to their original file positions for review. The contents search checks the first 15 pages, extending forwards in three-page windows if contents reaches the boundary (up to 15 additional pages). Otherwise it checks the final pages in backwards-growing three-page windows (up to 15), stopping when a complete contents list is found. `pdftotext -layout` supplements saved extraction without re-running OCR. One tier call uses the contents hierarchy, printed page spans and whole-book size; it must not expand a song/story category into all its numbered pieces. All saved headings remain available for local matching; only ambiguous matches go to the model. Consecutive selected headings with no included body text between them are folded into the following chapter, preserving the grouping text at that chapter's start; genuine short chapters with body text remain separate. The parsed contents is stored as `chapterProposal.toc` with file-local source pages and shown in the modal banner. Proposals leave existing chapters and extraction intact until the user applies them. Proposal titles flow through `applyChapterBoundaries` to `sliceChaptersAtIndices`. Applying boundaries queues `indexBook` after the new chapter rows are saved, so their search chunks and local embeddings refresh automatically. Calls use reasoning off and a 600s timeout.
...
- **Always `--disable_ocr`.** Marker is a layout engine here and nothing else: its OCR threw the recognition away after every run and Surya can only give line-level boxes. A scan is read by the `ocrTextLayer` step first, and marker reads the searchable copy that step wrote — `extractPdf` refuses a PDF with no text layer rather than spending half a minute discovering it
...
- **`skipSynthesis` is an instruction, not a state.** `workers/extract.ts` reads it once, to decide whether new chapters are born `pending` (and queued for synthesis) or `suspended`. Since 2026-09-07 every extraction path sets it (uploads send `skipSynthesis=true`; `books.retry` and `bookFiles.reExtractSelected` write it on the book and its files): extraction only produces the structure, and narration is a separate decision taken from the chapter tray after the chapters have been looked at, with the voice chosen there. Starting synthesis from extraction produced an M4B nobody asked for and spent hours before the structure was checked. The JSON API's `synthesize: true` and MCP `upload_book` with `skipSynthesis: false` are the two paths that still queue straight to TTS — both opt-in; `upload_book` defaulted to the unattended pipeline until 2026-09-25, when a book made from the assistant panel narrated all sixteen chapters the moment the extract card ran, and `extract_book` now writes `skipSynthesis` on the book before it starts, whatever the upload said.
- **Try one page** (`/books/:id/ocr?file=N`, `pages/OcrTryPage.tsx`, reached from the engine choice in `ExtractModal`) is the full-screen comparison the design brief asked for: the rendered page with Tesseract's doubted words shaded over it, a Tesseract card (a second, per-word confidence, whole-book time extrapolated from this page × page count) and a Surya card that only runs on request and streams lines over SSE with Stop. Choosing an engine writes `books.updateSettings` immediately; "Back to Extract" returns to `/books/:id?extract=1`, which `BookDetail` reads once to reopen the modal. The mock's tokens `--lowconf` / `--lowconf-ring` live in `styles.css`.
- **`ocrEngine` / `llmChapterDetection` / `language` are book properties, not app settings** — they describe the source and its text, outlive any single run, and live in `ExtractModal`, laid out in the order things happen — pictured pages read, table of contents, then language and what follows extraction — in plain words (no "Marker" on screen) rather than in the toolbar. The three re-extract buttons collapsed into one scope choice there. **Every scope is destructive** — including "chapter boundaries only", which `workers/redetect.ts` implements by deleting chapters, assemblies, documents and chapter audio — so each one names what it replaces and a tick-box gates the run whenever chapters exist.
- **`books.language` is filled from the text, never by an LLM.** `lib/detect-language.ts` (franc-min, a local trigram detector restricted to the app's own language list) sets it once when it is empty — after raw text lands, after an OCR copy lands, and in the upload dialog from the pages pdf.js sampled — and logs "Language from the text". A language someone set is never touched. An AI detector was built earlier and removed: the LLM is an addition, never a dependency, and this one needs no model.
...
- **Destructive UI actions confirm via native `confirm()`** (apply chapter boundaries, delete audio, delete folder…), not custom modals. The one exception is deleting a chat conversation, which the design gave a `Modal` so it can say what goes and what stays. Browser automation dismisses these by default — Playwright needs `page.once("dialog", d => d.accept())` before the click.
