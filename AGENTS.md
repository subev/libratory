...
packages/web/src/
  ...
  pages/
    ...
    Chat.tsx            Library chat: useChat + streaming /chat, folder (?folderId=) / book (?bookId=) scoping, source list, saved answers
...
**Frontend** (`pages/Chat.tsx`, `components/chat/`): `useChat` + `DefaultChatTransport` against `/chat` (scope + model sent per message in the request body), folder-scope dropdown, model toggle, tool calls rendered as collapsed "Searched: …" lines, `[c_N]` markers rewritten to `[n]` numbering matching the source list (`SourceList.tsx`: grouped under the book's title, each row the chapter and page plus how the passage starts — a dozen citations of one chapter were a dozen identical truncated pills). Row targets: a narrated chapter → the reader at the cue, `/books/:id/read?chapter=<index>&t=<ms>`, with the PDF kept beside it as a second button; otherwise raw or chapter with a resolvable file+page → `PdfPreviewModal` (`/pdf/:fileId#page=N`), else `/books/:bookId`; translation/variant → `/books/:bookId?variant=<key>`. `?t=` is where an unplayed chapter starts in `Reader.tsx` — the follow-scroll lands on whatever cue holds that moment.
