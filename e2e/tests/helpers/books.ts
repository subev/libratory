...
export async function uploadFixtureBook(page: Page, { waitForIndex = false } = {}) {
  ...
  // Scoped to the dialog: the assistant panel beside the library carries a file input of its own
  await page.getByTestId("upload-modal").locator('input[type="file"]').setInputFiles(FIXTURE_PDF);
...
export async function exportAs(page: Page, format: "epub-sync" | "m4b" | "epub" | "pdf") {
  ...
  // Target the chapter tray while navigation finishes; Outputs has another Export button.
